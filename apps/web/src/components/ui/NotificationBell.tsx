...
export default function NotificationBell() {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(!open)} className="relative p-2 text-slate-400 hover:text-[#0134BD] transition-colors">
        <Bell size={20} />
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-navy-800 rounded-xl shadow-xl border border-white/10 z-50">
          ...
          <div className="max-h-80 overflow-y-auto">
            ...
            {notifications.map(n => (
              <Link key={n.id} to={n.link || '#'} onClick={() => { markRead(n.id); setOpen(false) }} className={`flex items-start gap-3 p-3 hover:bg-white/5 transition-colors ${!n.read ? 'bg-blue-50/50' : ''}`}>
                <span className="text-lg flex-shrink-0">{typeIcons[n.type] || '📌'}</span>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm ${!n.read ? 'font-semibold text-white' : 'text-slate-400'}`}>{n.title}</p>
