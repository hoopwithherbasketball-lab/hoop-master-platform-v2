    ...
    <PageShell title={`${player.name} - Player Profile`} description={`${player.position} - Class of ${player.gradYear} - ${player.location}`}>
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex items-center gap-3 justify-center md:justify-start flex-wrap mb-1">
              <h1 className="text-3xl font-bold text-[#121B47]">{player.name}</h1>
              <VerifiedBadge level="elite" size="md" />
              <span className={`text-xl font-bold ${scoreColor(evalData.overall)}`}>{evalData.overall}</span>
            </div>
            <p className="text-lg text-gray-600 mb-2">{player.position} - Class of {player.gradYear}</p>
      ...
      <div className="bg-white rounded-lg shadow-md p-8 mb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {player.stats.map(stat => <div key={stat.label} className="bg-gray-50 rounded-lg p-4 text-center"><div className="text-3xl font-bold text-[#0134BD]">{stat.value}</div><div className="text-sm text-gray-600 mt-1">{stat.label}</div></div>)}
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-8 mb-8">
        <h2 className="text-2xl font-bold text-[#121B47] mb-6 flex items-center gap-2"><Award size={22} /> Evaluation Summary</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {evalData.categories.slice(0, 8).map(cat => (
            <div key={cat.label} className="bg-gray-50 rounded-lg p-4 text-center">
