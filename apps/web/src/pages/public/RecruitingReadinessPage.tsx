  ...
  return (
    ...
    >
      ...
      <section className="mb-12">
        <h2 className="text-3xl font-bold text-center text-[#121B47] mb-8">What Coaches Evaluate</h2>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {evaluationFactors.map((factor, index) => (
            <div key={factor.title} className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-lg font-semibold text-[#121B47]">{factor.title}</h3>
      ...
      <section className="bg-gray-50 p-8 rounded-lg mb-12">
        <h2 className="text-3xl font-bold text-center text-[#121B47] mb-8">Recruiting Readiness Checklist</h2>
        <div className="max-w-2xl mx-auto">
          <div className="mb-6">
            <div className="flex justify-between items-center mb-4">
              <span className="text-lg font-semibold text-[#121B47]">Progress</span>
          ...
          <div className="space-y-3">
            {checklistItems.map((item, index) => (
              <label key={index} className="flex items-center p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer">
                <input
                  type="checkbox"
                  checked={checkedItems[index]}
                  onChange={() => handleCheckChange(index)}
                  className="mr-3 w-5 h-5 text-[#0134BD] focus:ring-[#0134BD] border-gray-300 rounded"
                />
      ...
      <section className="mb-12">
        <h2 className="text-3xl font-bold text-center text-[#121B47] mb-8">Grade-by-Grade Timeline</h2>
          ...
          {timeline.map((stage, index) => (
            <div key={stage.grade} className="flex items-start">
              <div className="flex-shrink-0 w-24 text-center">
                ...
                <div className="text-sm font-semibold text-[#121B47]">{stage.grade}</div>
              </div>
              <div className="flex-grow bg-white p-6 rounded-lg shadow-md ml-6">
                <h3 className="text-xl font-semibold text-[#121B47] mb-2">{stage.focus}</h3>
