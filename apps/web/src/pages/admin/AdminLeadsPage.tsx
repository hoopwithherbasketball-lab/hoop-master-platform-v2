          ...
          {leads.map(l => (
            <div key={l.id} className="bg-navy-800 rounded-xl shadow-sm p-4 flex items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-3 flex-shrink-0">
                <span className={`px-2.5 py-1 rounded-full text-xs font-medium capitalize ${l.status === 'new' ? 'bg-blue-500/20 text-blue-700' : l.status === 'contacted' ? 'bg-amber-100 text-amber-700' : l.status === 'qualified' ? 'bg-purple-100 text-purple-700' : l.status === 'converted' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>{l.status}</span>
              </div>
            </div>
          ))}
