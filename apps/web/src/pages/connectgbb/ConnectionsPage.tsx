...
const statusColors: Record<string, string> = { approved: 'bg-green-100 text-green-700', pending: 'bg-yellow-100 text-yellow-700' }
...
export default function ConnectionsPage() {
  ...
  return (
    <PageShell title="My Connections" description="Manage your network of coaches, players, and programs." badge="ConnectGBB">
      {loading ? (
        <div className="animate-pulse space-y-4">{[1, 2, 3].map(i => <div key={i} className="bg-white p-6 rounded-lg shadow-md h-20" />)}</div>
      ) : (
        <div className="max-w-2xl mx-auto space-y-3">
          {connections.map(c => (
            <div key={c.id} className="bg-white p-4 rounded-lg shadow-md flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-[#0134BD] rounded-full flex items-center justify-center text-white font-semibold">
                  {c.displayName[0]}
                </div>
                <div>
                  <p className="font-semibold text-[#121B47]">{c.displayName}</p>
          ...
          {connections.length === 0 && <p className="text-center text-gray-500 py-12">No connections yet.</p>}
        </div>
      )}
    </PageShell>
  )
