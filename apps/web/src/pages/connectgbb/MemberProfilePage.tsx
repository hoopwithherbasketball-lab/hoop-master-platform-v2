...
export default function MemberProfilePage() {
  ...
  if (!profile) return <PageShell title="Not Found" description="Member not found." badge="EliteGBB"><Link to="/elitegbb" className="text-[#0134BD] hover:underline" data-testid="elitegbb-member-not-found-back-link">← Back to EliteGBB</Link></PageShell>

  return (
    <PageShell title={profile.displayName} description={`${profile.role} • ${profile.location}`} badge="Profile">
      <Link to="/elitegbb/feed" className="inline-flex items-center gap-1 text-sm text-slate-400 hover:text-[#0134BD] mb-2" data-testid="elitegbb-member-back-to-feed-link">← Back to Feed</Link>
      <div className="max-w-2xl mx-auto">
        <div className="bg-navy-800 rounded-xl shadow-md border border-white/10 p-6 text-center" data-testid="elitegbb-member-profile-card">
          ...
          <button className="mt-5 bg-[#0134BD] text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-[#002a80] transition-colors flex items-center gap-2 mx-auto" data-testid="elitegbb-member-message-button">
