    ...
    <PageShell title="Training Hub" description="Video lessons, drill libraries, and skill tracks designed for elite development." badge="ConnectGBB">
      {loading ? (
        <div className="animate-pulse grid gap-6 md:grid-cols-2">
          {[1, 2, 3, 4].map(i => <div key={i} className="bg-white p-6 rounded-lg shadow-md h-40" />)}
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2">
          {tracks.map(track => (
            <div key={track.id} className="bg-white p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow border-l-4 border-[#0134BD]">
              ...
              <h3 className="text-lg font-semibold text-[#121B47] mb-1">{track.title}</h3>
