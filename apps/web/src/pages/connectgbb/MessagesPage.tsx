    ...
    <PageShell title="Messages" description="In-app messaging with coaches, scouts, and programs." badge="ConnectGBB">
      <div className="flex h-[600px] bg-white rounded-xl shadow-md overflow-hidden">
        <div className="w-80 border-r border-gray-200 flex flex-col">
          <div className="p-3 border-b border-gray-200 bg-gray-50"><h3 className="font-semibold text-[#121B47] text-sm">Conversations</h3></div>
          <div className="flex-1 overflow-y-auto">
            {conversations.map(c => (
              <button key={c.id} onClick={() => setActiveConvoId(c.id)} className={`w-full text-left p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors ${activeConvoId === c.id ? 'bg-blue-50' : ''}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <p className="font-semibold text-[#121B47] text-sm truncate">{c.participantName}</p>
          ...
          {activeConvo ? (
            <>
              <div className="p-4 border-b border-gray-200 bg-gray-50 flex items-center gap-3">
                ...
                <div><p className="font-semibold text-[#121B47] text-sm">{activeConvo.participantName}</p><p className="text-xs text-gray-400">{activeConvo.participantRole}</p></div>
              </div>
                ...
                {activeConvo.messages.map(m => {
                  ...
                  return (
                    <div key={m.id} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[70%] p-3 rounded-xl ${isMe ? 'bg-[#0134BD] text-white' : 'bg-gray-100 text-gray-800'}`}>
              ...
              <div className="p-4 border-t border-gray-200">
                <div className="flex items-center gap-3">
                  <input value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === 'Enter' && sendMessage()} placeholder="Type a message..." className="flex-1 p-3 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#0134BD] text-sm" />
