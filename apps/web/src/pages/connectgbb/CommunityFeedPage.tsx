    ...
    <PageShell title="Community" description="Connect with players, coaches, and the Elite GBB community." badge="ConnectGBB">
      {loading ? (
        ...
        <div className="max-w-2xl mx-auto space-y-4">
          <div className="bg-navy-800 p-4 rounded-lg shadow-md border border-white/10">
            <textarea value={newPost} onChange={e => setNewPost(e.target.value)} placeholder="Share something with the community..." rows={2} className="w-full resize-none outline-none text-gray-300 text-sm" />
          ...
          {posts.map(post => (
            <div key={post.id} className="bg-navy-800 p-6 rounded-lg shadow-md">
