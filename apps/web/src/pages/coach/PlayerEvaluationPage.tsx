    ...
    <DashboardLayout variant="coach" title="Player Evaluation" subtitle={`${evalData.playerName} — ${evalData.position} | Class of ${evalData.gradClass}`} action={
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-navy-800 rounded-xl shadow-sm p-6">
          ...
          <p className="text-gray-300 leading-relaxed">{evalData.scoutNotes}</p>
        </div>

        <div className="grid gap-4 md:grid-cols-3">
          <div className="bg-green-50 rounded-xl p-5 border border-green-200">
            <h3 className="font-bold text-green-800 mb-3 flex items-center gap-2"><Award size={16} /> Strengths</h3>
            <ul className="space-y-2">{evalData.strengths.map(s => <li key={s} className="flex items-center gap-2 text-sm text-green-400"><span className="w-1.5 h-1.5 bg-green-500 rounded-full" />{s}</li>)}</ul>
          </div>
          ...
          <div className="bg-blue-500/10 rounded-xl p-5 border border-blue-200">
        ...
        <div className="bg-navy-800 rounded-xl shadow-sm p-6">
          ...
          <div className="flex items-center gap-3">
            <input value={newNote} onChange={e => setNewNote(e.target.value)} placeholder="Add a referral note..." className="flex-1 p-3 border border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-[#0134BD] text-sm" />
