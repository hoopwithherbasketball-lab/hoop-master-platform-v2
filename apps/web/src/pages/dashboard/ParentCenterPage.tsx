    ...
    <DashboardLayout variant="player" title="Parent Center" subtitle="Manage player profiles, consent, and account activity.">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="bg-white rounded-xl shadow-md p-6">
          <h2 className="text-lg font-bold text-[#121B47] mb-4 flex items-center gap-2"><Shield size={18} /> Connected Players</h2>
            ...
            {players.map(p => (
              <div key={p.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-[#121B47]">{p.name}</p>
                    <p className="text-xs text-gray-500">{p.position} • Class of {p.gradClass} • {p.school}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  {p.consentStatus === 'approved' ? (
                    <span className="flex items-center gap-1 px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-medium"><CheckCircle size={12} /> Approved</span>
                  ) : (
                    <>
        ...
        <div className="bg-white rounded-xl shadow-md p-6">
          <h2 className="text-lg font-bold text-[#121B47] mb-4 flex items-center gap-2"><Activity size={18} /> Recent Activity</h2>
