...
interface OrderDetail {
  ...
  player_profile: { first_name: string; last_name: string } | null
}
...
export default function ServiceOrderDetailPage() {
  ...
  useEffect(() => {
    if (!orderId) return
    supabase.from('service_orders').select('id, status, created_at, due_at, intake_complete, service_offer:service_offer_id(name, description, price_cents), player_profile:player_profile_id(first_name, last_name)').eq('id', orderId).single().then(({ data }) => {
      if (data) {
        ...
        setOrder({
          ...
          player_profile: (Array.isArray(r.player_profile) ? r.player_profile[0] : r.player_profile) as OrderDetail['player_profile'],
        })
      }
      setLoading(false)
    })
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="card p-6 space-y-4">
            ...
            <div className="space-y-3 text-sm text-slate-400">
              ...
              <div className="flex justify-between"><span>Player</span><span className="text-white">{order.player_profile ? `${order.player_profile.first_name} ${order.player_profile.last_name}` : '—'}</span></div>
          ...
          <div className="card p-6 space-y-4">
            ...
            <p className="text-slate-400 text-sm">Your order is being reviewed. Contact us with any questions.</p>
          </div>
        </div>
      ) : (
