    ...
    <DashboardLayout variant="player" title="Player Intake" subtitle="Complete your player profile">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center gap-2 mb-8 overflow-x-auto pb-2">
          {STEPS.map((label, i) => (
            <div key={label} className={`flex-shrink-0 flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${i + 1 === step ? 'bg-[#0134BD] text-white' : i + 1 < step ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
        ...
        <div className="bg-white rounded-lg shadow-md p-6 space-y-6">
          {step === 1 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Player Information</h2>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Parent / Guardian</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {step === 3 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Team Context</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {step === 4 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Stats Snapshot</h2>
              <p className="text-gray-500">Current season statistics (leave blank if not available)</p>
          ...
          {step === 5 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Self Evaluation</h2>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">What do you take pride in?</label>
                <div className="flex flex-wrap gap-2">
                  {PRIDE_TAGS.map(tag => (
                    <button key={tag} type="button" onClick={() => togglePrideTag(tag)} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${data.pride_tags.includes(tag) ? 'bg-[#0134BD] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{tag.replace('_', ' ').toUpperCase()}</button>
                  ))}
                </div>
              </div>
          ...
          {step === 6 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Film & Links</h2>
          ...
          {step === 7 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Goals</h2>
          ...
          {step === 8 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Select Your Package</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {PACKAGES.map(pkg => (
                  <button key={pkg.id} type="button" onClick={() => update('package_selected', pkg.id)} className={`p-5 rounded-xl border-2 text-left transition-all ${data.package_selected === pkg.id ? 'border-[#0134BD] bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                    <h3 className="text-lg font-bold text-[#121B47]">{pkg.name}</h3>
                    <p className={`text-2xl font-bold my-2 ${pkg.id === 'free' ? 'text-purple-600' : 'text-[#FB6C1D]'}`}>{pkg.price === 0 ? 'FREE' : `$${pkg.price}`}</p>
                    <ul className="space-y-1 text-sm text-gray-600">{pkg.features.map(f => <li key={f} className="flex items-center gap-1"><span className="text-[#0134BD]">✓</span>{f}</li>)}</ul>
                  </button>
                ))}
              </div>
            </div>
          ...
          {step === 9 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold text-[#121B47]">Consent & Signature</h2>
              <label className="flex items-start gap-3">
                <input type="checkbox" className="mt-1 w-5 h-5 text-[#0134BD] border-gray-300 rounded" />
                <span className="text-sm text-gray-700">I consent to evaluation services and understand this is informational and not a guarantee of recruiting outcomes.</span>
              </label>
              <label className="flex items-start gap-3">
                <input type="checkbox" className="mt-1 w-5 h-5 text-[#0134BD] border-gray-300 rounded" />
                <span className="text-sm text-gray-700">I consent to the use of player photos, videos, and information for promotional and recruiting purposes.</span>
              </label>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-between mt-6">
          <button onClick={prev} disabled={step === 1} className="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 disabled:opacity-50">← Previous</button>
          {step < totalSteps ? (
...
function InputField({ label, type = 'text', value, onChange, placeholder }: { label: string; type?: string; value: string; onChange: (v: string) => void; placeholder?: string }) {
  return <div><label className="block text-sm font-medium text-gray-700 mb-1">{label}</label><input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full p-2 border border-gray-300 rounded-md focus:ring-[#0134BD] focus:border-[#0134BD]" /></div>
}

function TextAreaField({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return <div><label className="block text-sm font-medium text-gray-700 mb-1">{label}</label><textarea value={value} onChange={e => onChange(e.target.value)} rows={3} className="w-full p-2 border border-gray-300 rounded-md focus:ring-[#0134BD] focus:border-[#0134BD]" /></div>
}
