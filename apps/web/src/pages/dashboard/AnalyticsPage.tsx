...
function MiniBar({ value, max, color }: { value: number; max: number; color: string }) {
  return <div className="w-full bg-gray-100 rounded-full h-3 overflow-hidden"><div className={`h-3 rounded-full ${color}`} style={{ width: `${(value / max) * 100}%` }} /></div>
}
    ...
    <DashboardLayout variant="player" title="Analytics" subtitle="Track your performance trends across the season.">
      <div className="max-w-5xl mx-auto space-y-8">
            ...
            <div key={s.label} className="bg-white rounded-xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-[#121B47]">{s.label}</h3>
                </div>
                <span className="text-2xl font-bold text-[#121B47]">{s.data[s.data.length - 1]}</span>
              </div>
              <div className="space-y-3">
                {s.data.map((val, i) => (
                  <div key={i} className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-medium text-gray-600 w-10 text-right">{val}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          <h2 className="text-lg font-bold text-[#121B47] mb-4">Season-over-Season Trends</h2>
          <div className="space-y-4">
            {['2023-24', '2024-25', '2025-26'].map(season => {
              const seasonData = analytics.trends.filter(t => t.season === season)
              return (
                <div key={season} className="p-4 bg-slate-50 rounded-lg">
                  <p className="font-semibold text-[#121B47] mb-2">{season}</p>
