...
| Config | Agents must not modify critical configuration files (e.g., turbo.json, root and workspace package.json files, lockfiles, tsconfig.json, .gitignore), .github/workflows, or Cloudflare configs (e.g., wrangler.toml) without explicit authorization (e.g., via agent-config-auth label) | CI check / PR Review |
