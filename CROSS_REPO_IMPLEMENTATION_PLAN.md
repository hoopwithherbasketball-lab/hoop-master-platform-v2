...
## External surface decision (non-elitegbb)

| Surface | Decision | Rationale | Phase impact |
|---|---|---|---|
| `apps/partner-portal` | **Remain external to core MVP migration** (owned as adjacent surface, integration-only in MVP) | Exists as separate auth shell + Python backend; not required as a blocking dependency for public MVP/Page Builder/ConnectGBB critical path. | Keep out of Phases 7-11 feature migration scope except for auth/data interface contracts. |
| `apps/player-advantage` | **Remain external to core MVP migration** (owned as adjacent surface, defer deep migration) | Current target state is backend-model oriented and not part of the Phase 7-11 MVP route commitments. | Defer substantive migration to post-Phase 11 unless explicitly re-scoped by Overseer. |
| `apps/player-advantage-app` | **Not currently present in tracked files; treat as external/deferred** | Repo audit indicates this app is not currently present. | No Phase 7-11 implementation scope until artifact/source path is confirmed. |

This satisfies the Phase 4 requirement to explicitly decide which non-core surfaces remain external versus included in immediate MVP migration work.
